...
#---farmer registration------
def manufact_reg(request):
    
    profile_form = ManufactRegistrationForm(request.POST)
    ...
    if request.method == 'POST':
        ...
        #     return redirect(request, 'account/manufact_reg.html')
    ...
    return render(request, 'account/manufact_reg.html', context)        

#Agriculture_officer
def staff_officer_reg(request):
...
class Checkout(View):
    def post(self, request):
        ...
        email = EmailMultiAlternatives(
            ' Order Confirmation: from tatiHut.com, Thanks for purchasing our   products!! ',
            template,
...
class ViewManufact(DetailView):
   
    queryset = User.objects.all().select_related('userprofile')
    #  def get(self, request):
    #     users = User.objects.all().select_related('profile')
    #     return render(request, 'store/manufact_details.html', {'users': users})
    
    
    template_name = 'store/manufact_details.html'
